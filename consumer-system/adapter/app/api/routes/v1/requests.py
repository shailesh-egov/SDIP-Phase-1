...
from app.services.data_exchange_service import poll_provider_service_results, send_request_to_provider_service
    ...
    try:
        with SessionLocal() as session:
            ...
            for request_row in pending_requests:
                ...
                await poll_provider_service_results(request_id,request)
...
async def request_provider_verify(background_tasks: BackgroundTasks, request_data: dict ,request:Request, api_key: str = Depends(verify_api_key)):
    """
    Triggers a verification request to check inclusion errors with the provider system
    """
    logger.info("Received request for provider verification")
    try:
        ...
        # Process the request (e.g., send to provider  system)
        logger.info("Sending request to provider  system")
        result = await send_request_to_provider_service(request_data ,request)
        logger.info("Request to provider  system successful")
        return result

    except HTTPException as e:
        logger.error(f"Validation error in provider verification request: {str(e.detail)}")
        raise e
    except Exception as e:
        logger.error(f"Error in provider verification request: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))

# Search Route
@router.post("/provider/search")
async def request_provider_search(background_tasks: BackgroundTasks, request_data: dict ,request:Request, api_key: str = Depends(verify_api_key)):
    """
    Triggers a search request to identify exclusion errors with the provider system
    """
    logger.info("Received request for provider search")
    try:
        ...
        # Send request to provider system
        logger.info("Sending request to provider system")
        result = await send_request_to_provider_service(request_data , request)
        logger.info("Request to provider system successful")
        return result
    except Exception as e:
        logger.error(f"Error in provider search request: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
